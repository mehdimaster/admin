...
$(document).ready(function() {
    ...
    autoClose = true;
    type = 'success';
    notifContent = '<div class="alert alert-success media fade in"><p><strong>?????!</strong> ??? ?? ?????? ??? ????? ??? ?? ???????.</p></div>';
    method = 3000;
    position = 'top';
    container ='';
    style = 'topbar';
...
/*
topRight ,topCenter ,bottomRight ,topCenter ,center ,bottomCenter ,topLeft ,centerLeft ,bottomLeft
content
success ,info ,danger ,warning
method : false meaning click noty has clear . timer : 3000  . validation : ok cancle

* */
function generate(position, container, content, type, boxModel, method, confirm) {

    if(boxModel == 'basic'){
        content = '<div id="basic-preview" class="preview active" style="">\n' +
            '    <div class="alert media fade in alert-'+type+'">\n' +
            '        <p>'+content+'</p>\n' +
            '    </div>\n' +
            '</div>';
    }else if(boxModel == 'buttons-preview'){
        content = '<div id="buttons-preview" class="preview dis-none active" style="display: block;">\n' +
            '  <div class="alert media fade in alert-'+type+'">\n' +
            // '    <h4 class="alert-title">شما فایل خود را ذخیره نکرده اید</h4>\n' +
            '    <p>'+content+'</p>\n' +
            '    <p>\n' +
            '      <button type="button" data-id="" class="btn btn-sm btn-white btn-square m-t-10 m-r-10 confirm-btn" onclick="confirm();">تایید</button>   \n' +
            '      <button type="button" class="btn btn-sm btn-dark btn-square m-t-10 cancel-btn">انصراف</button>\n' +
            '    </p>\n' +
            '  </div>\n' +
            '</div>';
    }
